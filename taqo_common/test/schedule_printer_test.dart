...
Schedule _getSchedule(int scheduleType, List<SignalTime> signalTimes,
    {bool byDayOfMonth = false,
    int dayOfMonth = 1,
    int esmEndHour = 61200000 /*5PM*/,
    int esmFrequency = 8,
    int esmPeriodInDays = 0,
    int esmStartHour = 32400000 /*9AM*/,
    int nthOfMonth = 1,
    int repeatRate = 1,
...
void main() {
  test('Daily', () {
    expect(
        schedule_printer.toPrettyString(_getSchedule(Schedule.DAILY, [
          _getSignalTime(
              SignalTime.FIXED_TIME, _getFixedTimeMillisFromMidnight(_12AM))
        ])),
        'Daily at 12:00AM');
    expect(
        schedule_printer.toPrettyString(_getSchedule(Schedule.DAILY, [
          _getSignalTime(
              SignalTime.FIXED_TIME, _getFixedTimeMillisFromMidnight(_230AM)),
          _getSignalTime(
              SignalTime.FIXED_TIME, _getFixedTimeMillisFromMidnight(_315PM))
        ])),
        'Daily at 02:30AM,03:15PM');
    expect(
        schedule_printer.toPrettyString(_getSchedule(
            Schedule.DAILY,
            [
              _getSignalTime(SignalTime.FIXED_TIME,
                  _getFixedTimeMillisFromMidnight(_12PM)),
              _getSignalTime(SignalTime.FIXED_TIME,
                  _getFixedTimeMillisFromMidnight(_445PM))
            ],
            repeatRate: 3)),
        'Every 3 days at 12:00PM,04:45PM');
  });

  test('Weekday', () {
    expect(
        schedule_printer.toPrettyString(_getSchedule(Schedule.WEEKDAY, [
          _getSignalTime(
              SignalTime.FIXED_TIME, _getFixedTimeMillisFromMidnight(_12AM))
        ])),
        'Daily at 12:00AM');
  ...
  test('Weekly', () {
    expect(
        schedule_printer.toPrettyString(_getSchedule(
            ...
            weekDaysScheduled: 1)),
        'Weekly on Sun at 12:00AM');
    expect(
        schedule_printer.toPrettyString(_getSchedule(
            Schedule.WEEKLY,
            [
              _getSignalTime(SignalTime.FIXED_TIME,
                  _getFixedTimeMillisFromMidnight(_230AM)),
              _getSignalTime(SignalTime.FIXED_TIME,
                  _getFixedTimeMillisFromMidnight(_315PM))
            ],
            weekDaysScheduled: 9)),
        'Weekly on Sun,Wed at 02:30AM,03:15PM');
    expect(
        schedule_printer.toPrettyString(_getSchedule(
            Schedule.WEEKLY,
            [
            ...
            weekDaysScheduled: 34,
            repeatRate: 2)),
        'Every 2 weeks on Mon,Fri at 12:00PM,04:45PM');
  });
  ...
  test('Monthly dayOfMonth', () {
    ...
    expect(
        schedule_printer.toPrettyString(_getSchedule(
            ...
            byDayOfMonth: true,
            dayOfMonth: 20)),
        'Monthly on 20 at 02:30AM,03:15PM');
    expect(
        schedule_printer.toPrettyString(_getSchedule(
            Schedule.MONTHLY,
            [
  ...
  test('Monthly nthOfMonth', () {
    ...
    expect(
        schedule_printer.toPrettyString(_getSchedule(
            ...
            weekDaysScheduled: 20,
            nthOfMonth: 3)),
        'Monthly on 3rd Tue,Thu at 02:30AM,03:15PM');
    expect(
        schedule_printer.toPrettyString(_getSchedule(
            Schedule.MONTHLY,
            [
  ...
  test('ESM', () {
    ...
    expect(
        schedule_printer
            .toPrettyString(_getSchedule(Schedule.ESM, [], esmWeekends: true)),
