...
class Database {
  ...
  int execute(String query, {List params}) {
...
;        } else if (param is double) {
    ...
    // Return ID for last inserted row, else -1
    // TODO Can return number of rows changed for non-inserts
    int ret = -1;
    if (query.toLowerCase().trim().startsWith('insert')) {
      ret = bindings.sqlite3_last_insert_rowid(_database);
    }
